"""
core/edges/diagnostics.py
--------------------------
Diagnostic reporting models for DevBrain Edges.
"""
...
class EdgeDiagnostic(BaseModel):
    """Single diagnostic record emitted during edge construction or validation."""
...
class EdgeDiagnostics(BaseModel):
    """Aggregate diagnostic report for edge collection execution."""
    diagnostics: List[EdgeDiagnostic] = Field(
    ...
    @property
    def errors(self) -> List[EdgeDiagnostic]:
        return [d for d in self.diagnostics if d.severity == DiagnosticSeverity.ERROR]

    @property
    def warnings(self) -> List[EdgeDiagnostic]:
        return [d for d in self.diagnostics if d.severity == DiagnosticSeverity.WARNING]

    def add_error(self, message: str, file_path: Optional[str] = None, line: Optional[int] = None, code: Optional[str] = None) -> EdgeDiagnostics:
        diag = EdgeDiagnostic(
            message=message,
        ...
        return EdgeDiagnostics(diagnostics=self.diagnostics + [diag])

    def add_warning(self, message: str, file_path: Optional[str] = None, line: Optional[int] = None, code: Optional[str] = None) -> EdgeDiagnostics:
        diag = EdgeDiagnostic(
            message=message,
