"""
core/import_edges/diagnostics.py
---------------------------------
Diagnostic reporting models for Import Edge Builder.
"""
...
class ImportDiagnostic(BaseModel):
    """Single diagnostic record emitted during import extraction or resolution."""
...
class ImportEdgeDiagnostics(BaseModel):
    """Aggregate diagnostic report for import edge builder execution."""
    ...
    def add_error(self, message: str, file_path: Optional[str] = None, line: Optional[int] = None, code: Optional[str] = None) -> ImportEdgeDiagnostics:
        ...
        return ImportEdgeDiagnostics(diagnostics=self.diagnostics + [diag])

    def add_warning(self, message: str, file_path: Optional[str] = None, line: Optional[int] = None, code: Optional[str] = None) -> ImportEdgeDiagnostics:
        diag = ImportDiagnostic(
            message=message,
