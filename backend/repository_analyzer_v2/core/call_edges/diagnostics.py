"""
core/call_edges/diagnostics.py
-------------------------------
Diagnostic reporting models for Call Edge Builder.
"""
...
class CallDiagnostic(BaseModel):
    """Single diagnostic record emitted during call extraction or resolution."""
...
class CallEdgeDiagnostics(BaseModel):
    """Aggregate diagnostic report for call edge builder execution."""
    diagnostics: List[CallDiagnostic] = Field(
    ...
    @property
    def errors(self) -> List[CallDiagnostic]:
        return [d for d in self.diagnostics if d.severity == DiagnosticSeverity.ERROR]

    @property
    def warnings(self) -> List[CallDiagnostic]:
        return [d for d in self.diagnostics if d.severity == DiagnosticSeverity.WARNING]

    def add_error(self, message: str, file_path: Optional[str] = None, line: Optional[int] = None, code: Optional[str] = None) -> CallEdgeDiagnostics:
        diag = CallDiagnostic(
            message=message,
        ...
        return CallEdgeDiagnostics(diagnostics=self.diagnostics + [diag])

    def add_warning(self, message: str, file_path: Optional[str] = None, line: Optional[int] = None, code: Optional[str] = None) -> CallEdgeDiagnostics:
        diag = CallDiagnostic(
            message=message,
