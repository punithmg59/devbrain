"""
core/type_reference_edges/builder.py
-------------------------------------
TypeReferenceEdgeBuilder Facade Entrypoint for discovering and constructing Type Reference Edges.
"""
...
from core.type_reference_edges.extractor import TypeReferenceExtractor
from core.type_reference_edges.resolver import TypeReferenceResolver
...
class TypeReferenceEdgeBuilder:
    """
    Facade Entrypoint that converts SemanticRepository type references into an immutable EdgeCollection.
    """
    ...
    def build(self, semantic_repository: SemanticRepository) -> EdgeCollection:
        ...
        extracted_stmts = extractor.extract_type_references(semantic_repository)
        ...
        for idx, stmt in enumerate(extracted_stmts):
            try:
                res = resolver.resolve_type_symbol(stmt, semantic_repository)
                ...
                if res.is_resolved:
                    ...
                    diagnostics = diagnostics.add_warning(
                        message=f"Unresolved type reference '{stmt.referenced_type_raw}' in '{stmt.source_file_path}'.",
                        file_path=stmt.source_file_path,
                        code="WARN_UNRESOLVED_TYPE_REF"
                    )

                if stmt.is_generic:
                    generic_count += 1

                disc = f"type_ref_{idx}_{stmt.referenced_type_raw}_{stmt.context}"
                ...
                ev = EdgeEvidence(
                    ...
                    stage_name="Step 4.5 Type Reference Edge Builder",
                    builder_name="TypeReferenceEdgeBuilder",
                    confidence_source=res.resolution_strategy,
                    explanation=f"Type reference to '{stmt.referenced_type_raw}' resolved via '{res.resolution_strategy}'."
                )

                meta = EdgeMetadata(
                    language={"lang": stmt.language.value},
                    custom={
                        "referenced_type_raw": stmt.referenced_type_raw,
                        "context": stmt.context,
                        "is_generic": stmt.is_generic,
                        "is_nullable": stmt.is_nullable,
                ...
                edge = Edge(
                    ...
                    strength=EdgeStrength.WEAK if res.is_primitive else EdgeStrength.NORMAL,
                    ...
                    origin=EdgeOrigin(creator="devbrain.type_reference_edge_builder", stage="Step 4.5"),
                    version=EdgeVersion(),
                    metadata=meta
                )
                ...
                by_kind.setdefault(EdgeKind.TYPE_REFERENCE, []).append(edge.id)
                ...
                diagnostics = diagnostics.add_error(
                    message=f"Error constructing type reference edge for '{stmt.referenced_type_raw}': {str(e)}",
                    file_path=stmt.source_file_path,
                    code="ERR_TYPE_REF_EDGE_BUILD_FAILED"
                )

        coll_stats = EdgeStatistics(
            total_edges=len(edge_list),
            edges_by_kind_counts={"type_reference": len(edge_list)},
        ...
        TypeReferenceEdgeValidator.validate(collection)
