...
function getVerdictFromReport(report: EngineeringReport): { score: number; label: string; color: string } {
  // Extract risk score from hero section or sections
...
export default function ChangeIntelligenceReportView({
  report,
  timing,
  onCopyReport,
  onShare,
  ...
  const verdictIcon = getVerdictIcon(verdict.score)
  
  const hero = report.hero as Record<string, unknown> | null
  const confidence = hero?.confidence as number ?? 0.8
  const impact = hero?.impact_count as number ?? 0
  const analysisTime = timing?.total_ms ? (timing.total_ms / 1000).toFixed(2) : '0.00'
  
  const executiveSummary = hero?.executive_summary as string ?? report.title ?? 'No summary available'
  ...
  return (
    <div className="min-h-screen bg-[#09090b]">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className={`rounded-2xl border p-8 ${verdict.color}`}>
          <div className="flex items-start justify-between mb-6">
            <div className="flex items-center gap-4">
              <div className={`p-3 rounded-xl bg-gray-900/50 ${verdict.color.split(' ')[0]}`}>
                {verdictIcon}
              </div>
              <div>
                <h1 className="text-3xl font-bold text-white mb-1">{verdict.label}</h1>
                <p className="text-sm text-gray-400">Risk Assessment</p>
              </div>
            </div>
            <div className="text-right">
              <div className="text-5xl font-bold text-white tabular-nums">{verdict.score}</div>
        ...
        {(topCallers.length > 0 || criticalDeps.length > 0) && (
          <CollapsibleSection 
            ...
            <div className="space-y-4">
              {topCallers.length > 0 && (
                <div>
                  ...
                  <div className="space-y-2">
                    {topCallers.slice(0, 5).map((caller, i) => (
                      <div key={i} className="flex items-center gap-3 p-3 rounded-lg bg-gray-800/50">
                        ...
                        <div className="text-sm text-gray-200">{caller}</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {criticalDeps.length > 0 && (
                <div>
                  ...
                  <div className="space-y-2">
                    {criticalDeps.slice(0, 5).map((dep, i) => (
                      <div key={i} className="flex items-center gap-3 p-3 rounded-lg bg-gray-800/50">
                        <ArrowRight className="w-4 h-4 text-gray-500" />
                        <div className="text-sm text-gray-200">{dep}</div>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                ...
                <div className="text-sm text-gray-300">{report.intent ?? 'N/A'}</div>
              </div>
              <div>
              ...
              <div>
                <div className="text-xs text-gray-500 uppercase tracking-wider mb-1">Generated At</div>
                <div className="text-sm text-gray-300">{report.generated_at ?? new Date().toISOString()}</div>
              </div>
            </div>
          </div>
            ...
            <div className="mt-6 pt-6 border-t border-gray-800">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {Object.entries(timing).map(([key, value]) => (
                  <div key={key}>
                    <div className="text-[10px] text-gray-600">{key.replace('_ms', '')}</div>
                    <div className="text-sm text-gray-400">{value} ms</div>
                  </div>
                ))}
              </div>
            </div>
